...
1. Resolves the project root from `$Antigravity_PROJECT_DIR` (Cowork roots cwd at `.klodock`; the env var is the right answer).
...
How the agent-pipeline-antigravity plugin is organized, what runs where, and which
...
1. **Plugin layer** (`agent-pipeline-antigravity/`) — the slash commands, the
...
flowchart TB
    subgraph PluginLayer["Plugin layer (one install per machine)"]
        direction LR
        A1[".Antigravity-plugin/plugin.json"]
    ...
    ProjectLayer -- "/agent-pipeline-antigravity:run produces" --> RunLayer
```
...
v1.0 collapsed the run-time surface to one command: `/run "<short description>"`. Before the first pipeline stage executes, `/run` spawns a **manifest-drafter** subagent (`.pipelines/roles/manifest-drafter.md`). The drafter walks the project root for spec, release-plan, scope-lock, design-note, ADR, `Antigravity.md`, and ledger artifacts, then writes a populated `manifest.yaml` plus a `draft-provenance.md` audit trail. The user reviews the drafted YAML in chat and replies `APPROVE` to start the pipeline.

v1.1 fixes the install adapter v1.0 got wrong:

- **Plugin skills are namespaced.** Per the [Antigravity plugin docs](https://code.Antigravity.com/docs/en/plugins), all marketplace plugin skills are invoked as `/<plugin-name>:<skill-name>`. The bare `/run` form documented in v1.0 was never reachable. The canonical form is `/agent-pipeline-antigravity:run`.
...
The judge is invoked as a fresh Antigravity subagent. The orchestrator deliberately supplies only the manifest, the matched action policy, prior judge decisions for this run (so the judge can detect re-proposals of blocked actions), and the structured action proposal block.
...
```
agent-pipeline-antigravity/                        # the plugin
├── .Antigravity-plugin/
...
```
<your-project>/
├── Antigravity.md                            # scaffolded if absent
...
| Project conventions | `Antigravity.md` | Roles read this; the planner is required to honor it |
...
- Editing the manifest mid-run. The orchestrator treats the manifest as
  ...
  re-issue `/agent-pipeline-antigravity:run`.
...
sequenceDiagram
    autonumber
    participant U as User
    participant CC as Antigravity
    participant Plugin as agent-pipeline-antigravity plugin
    participant Proj as your project
    participant Runs as .agent-runs/&lt;run-id&gt;/

    U->>CC: /agent-pipeline-antigravity:pipeline-init
    ...
    CC->>Proj: scaffold .pipelines/, scripts/policy/, Antigravity.md
    CC->>U: orientation summary; suggest /agent-pipeline-antigravity:run

    U->>CC: /agent-pipeline-antigravity:run "short description"
    CC->>Plugin: read skills/run/SKILL.md + references/run.md
    CC->>CC: spawn manifest-drafter subagent (pre-stage)
    CC->>Proj: drafter walks spec/release-plan/scope-lock/Antigravity.md/...
...
- **Gate** — a checkpoint where the pipeline halts. Either
  ...
  non-drafting run.
- **Subagent** — a fresh Antigravity agent spawned by the orchestrator
