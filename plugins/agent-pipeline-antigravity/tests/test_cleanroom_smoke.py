...
3. Run `Antigravity --plugin-dir <plugin-tmp> --add-dir <fixture-tmp> -p` from
   the fixture-tmp, driving:
       turn 1: invoke the agent-pipeline-antigravity:pipeline-init skill
       turn 2: APPROVE
...
Requires: Antigravity CLI on PATH + ANTHROPIC_API_KEY in env. Skips otherwise.
...
def _scaffold_minimal_fixture(dest: Path) -> None:
    ...
    (dest / "README.md").write_text(
        textwrap.dedent(
            ...
            `agent-pipeline-antigravity:pipeline-init` end-to-end against a fresh
    ...
    (dest / "Antigravity.md").write_text(
        textwrap.dedent(
            """\
            # Antigravity.md — Cleanroom smoke fixture conventions
...
def _drive_pipeline_init(
    ...
    """Two-turn Antigravity -p flow: invoke skill, then APPROVE.

    Returns the two CompletedProcess results.
    """
    Antigravity = shutil.which("Antigravity") or "Antigravity"
...
def test_cleanroom_pipeline_init_scaffolds_into_real_project(tmp_path: Path) -> None:
    ...
    `Antigravity -p --plugin-dir` invocation. After APPROVE, assert that the
    ...
    """
    antigravity_bin = shutil.which("Antigravity")
    if antigravity_bin is None:
        pytest.skip("Antigravity CLI not on PATH")
    try:
        res = subprocess.run([antigravity_bin, "--help"], capture_output=True, text=True, errors="replace", timeout=10)
        if "plugin" not in res.stdout and "plugin" not in res.stderr:
            pytest.skip("Antigravity CLI does not support plugin subcommands/options")
    except Exception:
        pytest.skip("Could not run Antigravity --help to check plugin support")
