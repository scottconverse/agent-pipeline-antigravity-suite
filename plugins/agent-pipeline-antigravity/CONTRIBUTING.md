# Contributing to agent-pipeline-antigravity
...
- [Discussions](https://github.com/scottconverse/agent-pipeline-antigravity/discussions) — ideas, Q&A, show-and-tell.
- [Issues](https://github.com/scottconverse/agent-pipeline-antigravity/issues) — bugs and feature requests.
...
rm -rf ~/\.gemini/plugins/marketplaces/agent-pipeline-antigravity
ln -s /path/to/your/agent-pipeline-antigravity ~/\.gemini/plugins/marketplaces/agent-pipeline-antigravity

# Or just edit ~/\.gemini/plugins/installed_plugins.json to point at your clone.
```
...
- Your Cowork or Antigravity CLI version.
...
Open a discussion in the [Ideas](https://github.com/scottconverse/agent-pipeline-antigravity/discussions/categories/ideas) board first. The plugin's surface is deliberately small; not every reasonable idea fits the design contract. A discussion gets you faster feedback than an issue + PR cycle.
...
Drop a markdown file at `pipelines/roles/<role>.md`. The file is the role's complete spec — a fresh Antigravity session given just that file + the run context should be able to do the work. Use the existing role files as templates.
