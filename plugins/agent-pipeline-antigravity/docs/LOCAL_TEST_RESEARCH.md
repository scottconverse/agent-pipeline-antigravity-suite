...
the cleanroom-smoke test that drives `Antigravity -p` against a real model.
...
**Tier A (route `Antigravity -p` through a local-model proxy):** technically
...
Single-turn Anthropic-shape request → `antigravity-router` (npm
package `@musistudio/antigravity-router`) → Ollama OpenAI-compatible
endpoint → response back in Anthropic shape: ✔ verified end-to-end.

Built-in `Bash` tool through the same stack with `gpt-oss:20b`: ✔
model emitted a valid `tool_use` block, Antigravity CLI executed the Bash
call, the model received the tool result and reported correctly.

Working `~/\.gemini-code-router/config.json` for the trivial path:
...
Antigravity -p --model haiku 'Use the Bash tool to run: echo OK'
```
...
Antigravity haiku 4.5 always sends an extended-thinking parameter in the
request. `antigravity-router`'s OpenAI transformer forwards it. Ollama
...
`--bare`, `--model Antigravity-haiku-3-5`, multiple transformer combos
...
the full cleanroom-smoke flow (two-turn `Antigravity -p`: invoke the
...
test was halted. Antigravity CLI drops the SSE stream and ccr can't keep
...
`Antigravity` CLI evolution.

## Conditions under which to revisit Tier A

1. `antigravity-router` (or a successor) ships SSE forwarding that
...
3. Antigravity CLI grows native OpenAI-compatible provider support (today
...
2. **The LLM/CLI fails to invoke the skill correctly** (skill
   markdown is unreadable, Skill-tool dispatch broken, Antigravity CLI
   silently swallows the call). This still requires a real model;
   cleanroom-smoke remains the test that catches it for API-key
   holders. `Antigravity plugin list ✔ enabled` + manual Cowork-app
