...
**Subject:** `agent-pipeline-antigravity` v0.5.2 → v1.0.0.
...
**Friction 1 — install path.** README said: `/plugin install scottconverse/agent-pipeline-antigravity`. Scott typed `/plugin marketplace add scottconverse/agent-pipeline-antigravity` and got `/plugin isn't available in this environment.` The v0.5.2 plugin had no documented Cowork install path.

**Friction 2 — restart-required not surfaced.** After a file-level install completed cleanly (commit `79db3a7` cloned into `~/\.gemini/plugins/marketplaces/`, plus three JSON config patches), the slash commands `/pipeline-init`, `/run`, etc. did not appear in the current Cowork session. The product copy gave no signal that a restart was required.
...
| F-08 | MAJOR | Predecessor naming bleed-through (old `agentic-pipeline` install conflicts with new `agent-pipeline-antigravity`). |
...
| F-11 | MINOR | `Antigravity.md` scaffolding overwrites blind. |
| F-12 | MINOR | Glossary is undermaintained. |
| F-13 | MINOR | No example `.agent-runs/` in the repo. |

The full audit text (with severity definitions, evidence citations, and remediation per finding) lives in the [original session transcript on the v0.5.2 → v1.0 redesign turn](https://github.com/scottconverse/agent-pipeline-antigravity/commits/v1) — the audit is preserved here as a summary; the receipts are in the commit messages and CHANGELOG.
...
Every install path documented for Cowork users first. The CLI path remains supported via a one-line convenience note. The Cowork bootstrap prompt (paste into any Antigravity session) is the primary documented install method.

**Addresses:** F-01, F-02 (BLOCKER), F-08 (MAJOR).

### 2. Spec-aware drafting, not spec-blind authoring

The plugin reads the project's existing artifacts (spec, release plan, scope-lock, design notes, ADRs, ledgers, `Antigravity.md`) and drafts the manifest. The user reviews a filled YAML in chat and replies `APPROVE`. The user does not hand-author from blank.
...
>
> Test surface: schema-validator tests (added in v1.0), plus the new manifest-drafter integration fixtures (added in v1.0 under `tests/fixtures/`). A fully-automated drafter integration test that spawns a real Antigravity session is out of scope for v1.0.
>
...
>
> Blind spots: drafter quality is uneven across projects (sloppy artifacts produce sloppy drafts). v1.0 surfaces this honestly in copy ("the more structured your docs, the better this works") and provides the `## Pipeline drafter notes` Antigravity.md section as an explicit override path.
...
The v1 branch lives at https://github.com/scottconverse/agent-pipeline-antigravity/tree/v1. The v1.0.0 tag is cut from `1bf5ed1` (or the latest Phase 3 SHA, whichever is HEAD at tag time).
...
If a future change wants to revisit any of the four load-bearing decisions, the contributor should open a discussion at https://github.com/scottconverse/agent-pipeline-antigravity/discussions with evidence that the new design addresses the same friction in a better way.
