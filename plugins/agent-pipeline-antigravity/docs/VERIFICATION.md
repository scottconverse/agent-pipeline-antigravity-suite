...
End-to-end test receipts for [agent-pipeline-antigravity](https://github.com/scottconverse/agent-pipeline-antigravity). This file's narrative was captured at v1.1.1; the harness has expanded substantially through v1.3.x and v2.0 (hook layer, memory layer, directive contracts). The receipts below document the v1.1.1 baseline; the v2.0 test count is in CHANGELOG.md. Ran on Windows 11 Pro, MSYS Git Bash, Antigravity CLI 2.1.87 / Antigravity Desktop 2.1.128.
...
$ Antigravity plugin list
  ❯ agent-pipeline-antigravity@agent-pipeline-antigravity
    Version: 1.1.1
    Scope: user
    Status: ✔ enabled

$ Antigravity plugin validate .Antigravity-plugin/plugin.json
✔ Validation passed

$ Antigravity plugin validate .Antigravity-plugin/marketplace.json
✔ Validation passed
```
...
- 3 from `test_cleanroom_install.py` (cleanroom-load — `Antigravity --plugin-dir` load test, manifest validate from cleanroom copy, structure check from cleanroom copy)
...
- Runs `Antigravity --plugin-dir <plugin> -p` from the fixture, two turns: invoke `agent-pipeline-antigravity:pipeline-init`, then `APPROVE`.
...
# 4 turns, all Sonnet
Antigravity -p --session-id "$SID" --model sonnet \
  'Use the Skill tool to invoke "agent-pipeline-antigravity:pipeline-init".'
Antigravity -p --resume "$SID" --model sonnet 'APPROVE'

Antigravity -p --resume "$SID" --model sonnet \
...
- **B-4 (no live test/lint/type output).** `implementation-report.md:94` says "Tests would pass: ImportError resolves when store.py and router.py exist." Global `Antigravity.md` §Verification: *"'It should work' is not evidence."* The fixture environment can't satisfy this; flagged for live-env follow-up.
...
For routine runs Haiku is fine and cheap; for runs where you want eligibility checks to be meaningful, use Sonnet (`--model Antigravity-sonnet-4-6` at session start).
...
```bash
# Prereqs: Antigravity CLI, ANTHROPIC_API_KEY, ~$15 of API budget for Sonnet
# (or ~$2 for Haiku — but Sonnet is what we documented above)

git clone https://github.com/scottconverse/agent-pipeline-antigravity.git
cd agent-pipeline-antigravity
