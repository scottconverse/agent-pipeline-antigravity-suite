...
A structured per-project memory file the manifest can reference. Lives at `.pipelines/project-memory.yaml`. Captures things that Antigravity.md doesn't have a natural home for: which paths are "stable surface" vs. "rough edge," recurring drift patterns the auditor has surfaced, project-specific failure receipts that should warn future executor runs.

**Why now:** v0.3's audit-handoff discipline produces a stream of drift findings; they currently land in ad-hoc Antigravity.md edits or scattered ADRs. A structured home would let the v0.4 judge, the policy stage, and the verifier all reference the same record.
