...
A persistent project-level memory the manifest can reference, separate from Antigravity.md and ADRs. Use case: the executor and verifier repeatedly need to know which paths are "stable surface" (carefully reviewed) vs. "rough edge" (work-in-progress). Currently this lives in ad-hoc Antigravity.md sections; a structured memory file would let the policy stage and the v0.4 judge reference it.
...
`/pipeline-init` today asks for a PRD and scaffolds Antigravity.md. The next step is the manifest — currently the operator fills it by hand. Sketch: a `/manifest-from-prd` command that reads the PRD, the recently-touched code, and proposes a draft manifest for the operator to edit. Reduces the manifest authoring time from 10 minutes to 2.
...
- "Use a different LLM." Out of scope; the plugin is Antigravity-shaped.
