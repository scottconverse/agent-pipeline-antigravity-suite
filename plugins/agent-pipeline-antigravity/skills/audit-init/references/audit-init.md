...
Optional wiring:
- For Antigravity in either role: a Antigravity memory feedback file pointing at the right artifact.
...
- Optional update to project-level `Antigravity.md` or runtime-equivalent project-context file declaring the discipline.
...
- **Question:** `Which AI system implements code in this project?` Options: `Antigravity`, `Other (name it)`.
- **Question:** `Which AI system audits in this project?` Options: `Antigravity`, `Other (name it)`. (If same as implementer, this is a single-agent project — see Step 6.)
...
- `<IMPLEMENTER_AGENT>` — `Antigravity` or the operator's named other AI.
- `<AUDITOR_AGENT>` — `Antigravity` or the operator's named other AI.
...
Scaffolded by /audit-init from scottconverse/agent-pipeline-antigravity v0.3+.
...
### If Antigravity is the auditor
Create a memory feedback file at `~/\.gemini/projects/<encoded-cwd>/memory/feedback_<project>_audit_protocol.md` with:

```markdown
---
name: <PROJECT_NAME> audit protocol — Antigravity is auditor
...
Append a pointer line to `~/\.gemini/projects/<encoded-cwd>/memory/MEMORY.md` under the appropriate section.

### If Antigravity is the implementer
Create a memory feedback file at `~/\.gemini/projects/<encoded-cwd>/memory/feedback_<project>_5_lens_self_audit.md` with:

```markdown
---
name: <PROJECT_NAME> 5-lens self-audit — Antigravity is implementer
...
### If a non-Antigravity AI is in either role
...
## Step 7 — Update project-level Antigravity.md or AGENTS.md

If the project has a `Antigravity.md` or `AGENTS.md`, ask before editing. If approved, add a "HARD GATE — <PROJECT_NAME> Cross-Agent Audit Protocol" section pointing at the gate and protocol. Pattern:
