---
name: audit-init
description: Scaffold dual-AI audit-handoff infrastructure for a project — out-of-repo audit gate + protocol and in-repo 5-lens self-audit. For projects where one AI implements and a different AI audits. Invoked as /agent-pipeline-antigravity:audit-init.
---

# Audit-init

Follow the canonical workflow in `references/audit-init.md`. That document is the single source of truth for the input collection, the three artifacts produced, per-agent wiring, single-agent fallback, and project-level Antigravity.md updates.
...
- For non-Antigravity AI roles, print the file paths + integration pattern; do not attempt to wire the other runtime.
