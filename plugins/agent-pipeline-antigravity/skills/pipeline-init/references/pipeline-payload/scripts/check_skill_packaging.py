"""Validate installable Antigravity skills are self-contained.

Antigravity's plugin loader copies each ``skills/<name>/`` directory into a
...
# (docs/, tests/, Antigravity.md, etc.) because those live in the user's repo, not
...
def check_installed_copy(skill_dir: Path) -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="agent-pipeline-antigravity-skill-install-") as tmp:
