---
name: run-autonomous
description: (Deprecated in v1.3.0 — use /agent-pipeline-antigravity:run instead.) The v1.2.x grant-based autonomous mode is gone. The standard /run now uses fast modal AskUserQuestion gates (one click each) and auto-promotes evidence-driven when all checks pass.
---
...
- **There is no separate autonomous mode.** Just `/agent-pipeline-antigravity:run`.

## What to do instead

Run `/agent-pipeline-antigravity:run "<task description>"`. The flow:
...
If you typed `/agent-pipeline-antigravity:run-autonomous` expecting the old behavior, please use `/agent-pipeline-antigravity:run` instead.
